...
public interface EventRepository extends JpaRepository<Event, Long> {
	...
	@Query("SELECT e FROM Event e WHERE " +
			"(:users IS NULL OR SIZE(:users) = 0 OR e.initiator.id IN :users) AND " +
			"(:states IS NULL OR SIZE(:states) = 0 OR e.state IN :states) AND " +
			"(:categories IS NULL OR SIZE(:categories) = 0 OR e.category.id IN :categories) AND " +
